document.addEventListener("DOMContentLoaded", function() {
    ...
    if (queryPopup || storyPopup) {
        ...
        if (storyForm) {
            storyForm.addEventListener("submit", function(e) {
                ...
                .then(data => {
                    if (data.status === 'success') {
                        // Remove the alert and replace with inline message
        ...
        fetch('/get_queries')
            ...
            .then(data => {
                ...
                if (data.status === 'success' && data.queries && data.queries.length > 0) {
                    // Filter to only show public queries
        ...
        card.innerHTML = `
            <div class="query-header">
                <div class="query-meta">
                    ...
                    <div class="user-info">
                        <span class="query-username">${query.username}</span>
    ...
    function createStoryCard(story) {
        ...
        card.innerHTML = `
            <div class="story-header">
                <span class="story-author">${story.username}</span>
                <span class="story-date">${story.created_at}</span>
            </div>
    ...
    // Initial load
    const activeSection = document.querySelector('.content-section.active').id;
    if (activeSection === 'helpline-content') {
        loadHelplines();
